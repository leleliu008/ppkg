...
int main(int argc, char * argv[]) {
    char * const compiler = getenv("PROXIED_TARGET_CC");

    if (compiler == NULL) {
        fprintf(stderr, "PROXIED_TARGET_CC environment variable is not set.\n");
        return 1;
    }

    if (compiler[0] == '\0') {
        fprintf(stderr, "PROXIED_TARGET_CC environment variable value should be a non-empty string.\n");
        return 2;
    }
    ...
    char * const baseArgs = getenv("WRAPPER_TARGET_CCFLAGS");
