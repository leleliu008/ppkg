...
int ppkg_download(const char * url, const char * uri, const char * expectedSHA256SUM, const char * outputPath, const bool verbose) {
    ...
    if (outputPath == NULL || outputPath[0] == '\0' || strcmp(outputPath, "-") == 0 || strcmp(outputPath, "/dev/stdout") == 0) {
        int ret = ppkg_http_fetch_to_stream(url, stdout, verbose, verbose);

        if (ret != PPKG_OK) {
            if ((uri != NULL) && (uri[0] != '\0')) {
                ret = ppkg_http_fetch_to_stream(uri, stdout, verbose, verbose);
            }
        }
    ...
    if (strcmp(outputPath, "/dev/stderr") == 0) {
        int ret = ppkg_http_fetch_to_stream(url, stderr, verbose, verbose);

        if (ret != PPKG_OK) {
            if ((uri != NULL) && (uri[0] != '\0')) {
                ret = ppkg_http_fetch_to_stream(uri, stderr, verbose, verbose);
            }
        }
    ...
    if (strcmp(outputPath, ".") == 0 || strcmp(outputPath, "./") == 0) {
        int ret = ppkg_examine_filename_from_url(url, outputFilePath, PATH_MAX);
    ...
    if (strcmp(outputPath, "..") == 0 || strcmp(outputPath, "../") == 0) {
        ...
        int ret = ppkg_examine_filename_from_url(url, outputFilePath + 3, PATH_MAX - 3);
    ...
    if (outputPath[outputPathLength - 1U] == '/') {
        ...
        int ret = ppkg_examine_filename_from_url(url, outputFilePath + outputPathLength, PATH_MAX - outputPathLength);
    ...
    if (slashIndex > 0) {
        ...
        int ret = ppkg_mkdir_p(outputDIR, verbose);
