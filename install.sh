...
INSTALL_DIR=/usr/local/opt/zpkg
DEST_LINK_BIN=/usr/local/bin/zpkg
DEST_LINK_ZSH_COMPLETION=/usr/local/share/zsh/site-functions/_zpkg
...
main() {
    command -v zpkg    && die "zpkg is already installed."
    [ -d "$INSTALL_DIR" ] && die "zpkg is already installed. in $INSTALL_DIR"
    ...
    chown -R "$(own .)" .
    chmod 755 bin/zpkg
    chmod 444 zsh-completion/_zpkg
    
    if [ -f "$DEST_LINK_BIN" ] ; then
        die "$DEST_LINK_BIN is already exist."
    else
        [ -d '/usr/local/bin' ] || mkdir -p /usr/local/bin
        ln -s "$INSTALL_DIR/bin/zpkg" "$DEST_LINK_BIN" &&
    ...
    if [ -f "$DEST_LINK_ZSH_COMPLETION" ] ; then
        ...
        ln -s "$INSTALL_DIR/zsh-completion/_zpkg" "$DEST_LINK_ZSH_COMPLETION" &&
        IS_CREATED_BY_ME_LINK_ZSH_COMPLETION=true
    fi

    SUCCESS=true 
    success "Installed success."
    msg "${COLOR_PURPLE}Note${COLOR_OFF} : I have provide a zsh-completion script for ${COLOR_PURPLE}zpkg${COLOR_OFF}. when you've typed ${COLOR_PURPLE}zpkg${COLOR_OFF} then type ${COLOR_PURPLE}TAB${COLOR_OFF} key, it will auto complete the rest for you. to apply this feature, you may need to run the command ${COLOR_PURPLE}autoload -U compinit && compinit${COLOR_OFF}"
}
