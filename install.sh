...
# https://github.com/leleliu008/zpkg/blob/master/install.sh
...
main() {
    ...
    INSTALLED_NDK_PKG=$(command -v zpkg) || true

    if [ -n "$INSTALLED_NDK_PKG" ] ; then
        die "zpkg is already installed at $INSTALLED_NDK_PKG."
    fi

    VERSION='0.1.0'
    FILE_NAME="zpkg-${VERSION}.tar.gz"
    URL="https://github.com/leleliu008/zpkg/releases/download/v${VERSION}/${FILE_NAME}"

    INSTALL_BIN_DIR='/usr/local/bin'
    INSTALL_BIN_FILEPATH="$INSTALL_BIN_DIR/zpkg"

    INSTALL_ZSH_COMPLETION_DIR='/usr/local/share/zsh/site-functions'
    INSTALL_ZSH_COMPLETION_FILEPATH="$INSTALL_ZSH_COMPLETION_DIR/_zpkg"
    ...
    if [ -w "$INSTALL_BIN_DIR" ] ; then
        run      install -m 555 bin/zpkg "$INSTALL_BIN_FILEPATH"
    else
        run sudo install -m 555 bin/zpkg "$INSTALL_BIN_FILEPATH"
    fi
    ...
    if [ -w "$INSTALL_ZSH_COMPLETION_DIR" ] ; then
        run      install -m 644 zsh-completion/_zpkg "$INSTALL_ZSH_COMPLETION_FILEPATH"
    else
        run sudo install -m 644 zsh-completion/_zpkg "$INSTALL_ZSH_COMPLETION_FILEPATH"
    fi
    ...
    print "${COLOR_PURPLE}Note${COLOR_OFF} : ${COLOR_GREEN}$INSTALL_ZSH_COMPLETION_FILEPATH${COLOR_OFF} is a zsh-completion script for ${COLOR_PURPLE}zpkg${COLOR_OFF}. In zsh, when you've typed ${COLOR_PURPLE}zpkg${COLOR_OFF} then type ${COLOR_PURPLE}TAB${COLOR_OFF} key, it will auto complete the rest for you. to apply this feature, you may need to run the command ${COLOR_PURPLE}autoload -U compinit && compinit${COLOR_OFF}\n"
}
